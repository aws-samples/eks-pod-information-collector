...
function get_cluster_info() {
	...
	local CLUSTER_INFO
	CLUSTER_INFO=$(echo $CLUSTER_INFO | sed "s|}$|,\"serverVersion\": \"${CLUSTER_VERSION}\", \"clientVersion\": \"${KUBECTL_VERSION}\",\"iamARN\": \"${IAM_ARN}\"}|")
...
function get_volumes() {
	...
	if [[ ${COLLECT_EFS_CSI_LOGS} == 'YES' ]]; then
		...
		log "Getting EBS CSI Controller deployment logs of all containers"
		EFS_CSI_LOG_FILE=$(get_filename "efs-csi-${container}" "log")
	...
	for cm in ${CMS[*]}; do
		...
		if [[ ! $(kubectl get cm $cm -n "${NAMESPACE}") ]]; then #Check Associated ConfigMap JSON
